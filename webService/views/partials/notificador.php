 ...
 *
 * Uso desde JS (respuesta de API):
 *   window.notify('success', 'Producto agregado.');
 *   window.notify('error',   'No tienes permisos.');
 *   window.notify('warning', 'Revisa los campos.');
 *   window.notify('info',    'Procesando...');
 *
...
.ns-toast {
    ...
    font-family: var(--ns-font-body, 'Martian Mono', monospace);
    font-size: 0.85rem;
    color: var(--ns-color-text, #3d2b1f);
    pointer-events: all;
...
.ns-toast.ns-hiding {
    animation: ns-slide-out 0.2s ease forwards;
}
...
(function () {
    ...
    /**
     * Atajo para respuestas de la API BookArt.
     * Uso: window.notifyResponse({ success: true, message: '...' })
     */
    window.notifyResponse = function (data) {
        const type = data.success ? 'success' : 'error';
        window.notify(type, data.message ?? '');
    };
})();
</script>
